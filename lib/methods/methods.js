...
if (Meteor.isServer) {
  Meteor.methods({
    ...
    addVotedUsername: function (projectId, votedUser) {
      if (!Meteor.userId()) {
        throw new Meteor.Error('Not authorized')
      } else {
        Projects.update({_id: projectId}, { $addToSet: {voted: votedUser} })
      }
    },
    projectAuthorAwesomeScore: function (projectAuthorId) {
      if (!Meteor.userId()) {
        throw new Meteor.Error('Not authorized')
      } else {
        Meteor.users.update({_id: projectAuthorId}, { $inc: {'awesomeScore': +1} })
      }
    },
    projectAwesomeVote: function (projectId) {
      if (!Meteor.userId()) {
        throw new Meteor.Error('Not authorized')
      } else {
        Projects.update({_id: projectId}, { $inc: {'scores.awesomeScore': +1} })
      }
    }
